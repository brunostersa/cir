...
export const TIPO_PRODUTO_OPTIONS = [
  ...
  { value: 'papelaria', label: 'Papelaria Completa' },
...
export const ARTE_OPTIONS = [
  ...
  { value: 'arte_ia', label: 'Sim, tenho a arte criada com IA' },
...
export const INVESTIMENTO_OPTIONS = [
  ...
  { value: '500_2000', label: 'R$ 500 a R$ 2.000' },
  { value: '2000_10000', label: 'R$ 2.000 a R$ 10.000' },
...
// Faixa pré-definida OU valor exato digitado em "outro" — resolve pro texto
// final usado no resumo e no lead salvo.
export function getInvestimentoLabel(quiz) {
  if (quiz.investimento === 'outro') {
    return quiz.investimento_outro ? `R$ ${quiz.investimento_outro}` : ''
  }
  return labelFor(INVESTIMENTO_OPTIONS, quiz.investimento)
}
...
export function getQuizSummaryItems(quiz) {
  ...
  const investimentoLabel = getInvestimentoLabel(quiz)
  if (investimentoLabel) items.push({ label: 'Valor estimado total', value: investimentoLabel })
  return items
}
